...
- Уведомления о сделках спотового грид-бота MEXC: бот раз в `GRID_POLL_INTERVAL_MS` опрашивает `GET /api/v3/myTrades` по парам из `GRID_SYMBOLS` и присылает сообщение по каждой новой сделке всем, кто написал `/start`. У MEXC нет отдельного API для грид-ботов — уведомления приходят по всем сделкам аккаунта на указанных парах.
...
GRID_SYMBOLS=BTCUSDT,ETHUSDT
GRID_POLL_INTERVAL_MS=20000
```
...
- `GRID_SYMBOLS` — торговые пары грид-бота через запятую (например `BTCUSDT,ETHUSDT`). Если не задать — слежение за сделками отключено.
- `GRID_POLL_INTERVAL_MS` — интервал опроса MEXC в миллисекундах (по умолчанию `20000`).
